...
- 간략한 설명(실제는 좀 더 복잡하다)
예) 3.14 = 314 * 10^-2
[24비트(정수값 저장) = 가수부] [8비트(지수값 저장) = 지수부] = 32비트
...
- 개발자가 부동소수점을 사용할 때는 대략 다음의 기준을 삼으면 된다.
  부동소수점에서 소수점을 뗀 정수 값이 1600만을 넘지 않으면 올바르게
  메모리에 저장할 수 있다.
...
- 이렇게 안전하게 저장할 수 있는 부동소수점의 자릿수를 "유효자리수"라고 부른다.
...
32비트 유효자릿수 7자리, 64비트 유효자릿수 15자리!
  - 32비트보다 64비트 메모리가 두 배 큰(더 정밀한) 값을 저장할 수 있다.
  - 64비트는 두 배 정밀하다 => 배정도(double precision)
